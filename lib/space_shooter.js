...
function Ship() {
	...
	this.draw = function() {
		...
		$("#shipX").text(this.x.toFixed());
		$("#shipY").text(this.y.toFixed());
	};
	this.move = function() {	
		counter++;
...
function Game() {
	...
	this.init = function() {
		...
		if (this.bgCanvas.getContext) {
			...
			var shipStartY = this.shipCanvas.height/4*3 + imageRepository.spaceship.height*2 -20;
			this.ship.init(shipStartX, shipStartY, imageRepository.spaceship.width,
			               imageRepository.spaceship.height);

			this.shipStart = {x: shipStartX, y: shipStartY};
	...
	this.resetShip = function() {
		this.ship.addEvent({"type": "move", data: {x: this.shipStart.x, y: this.shipStart.y}});
	}
}
