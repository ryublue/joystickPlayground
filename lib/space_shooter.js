...
function Ship() {
	...
	this.draw = function() {
		...
		window.playground.ship.x = this.x;
		window.playground.ship.y = this.y;
		
		$("#shipX").text(window.playground.ship.x.toFixed());
		$("#shipY").text(window.playground.ship.y.toFixed());
	};
	this.move = function() {	
		counter++;
	
		var prevEvent;
		if (window.playground.events.length > 0) {
			this.context.clearRect(this.x, this.y, this.width, this.height);
			var event = window.playground.events.shift();
			this.x += event.data.normX;
			this.y -= event.data.normY;

			// remember previous event
			prevEvent = event;
		...
		{
			// if (prevEvent) {
			// 	this.x += prevEvent.data.normX;
			// 	this.y -= prevEvent.data.normY;
			// }
